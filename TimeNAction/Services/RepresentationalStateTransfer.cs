        ...
        protected virtual void CombineScripts(HttpContext context, bool isSaaS, string scriptName, string culture, string version)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsMobileClient)
                	...
                	sb.AppendFormat(String.Format("$(\'<link></link>\').appendTo($(\'head\')).attr({{ href: \'{0}/App_Themes/TimeNAction/" +
                                "_Theme_Plastic.css?{0}\', type: \'text/css\', rel: \'stylesheet\' }});", ApplicationServices.Version), baseUrl);
                try
                {
                ...
                sb.AppendFormat("var __targetFramework=\'4.5\';__timeZoneUtcOffset={0};__tf=4.0;__cothost=\'appfactor" +
                        "y\';__appInfo=\'IpacTimeNAction|{1}\';", ControllerUtilities.UtcOffsetInMinutes, context.User.Identity.Name);
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            if (!(String.IsNullOrEmpty(callback)))
            	...
            	if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("TimeNAction=typeof TimeNAction==\'undefined\'?{{}}:TimeNAction;TimeNAction.{0}=", instance);
                }
        }
